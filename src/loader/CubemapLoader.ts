...
export class CubemapLoader extends Middleware<GLCubemap> {
    ...
    add(
        key: string,
        levels: Array<{ right: string; left: string; top: string; bottom: string; back: string; front: string }>,
        config: TextureLoaderConfig = { id: 0 }
    ): Middleware<GLCubemap> {
        const resources = [];
    ...
    transform(...resources: Resource<HTMLImageElement>[]): GLCubemap {
        ...
        if (textureConfig.sampler) {
            if (
                textureConfig.sampler.minFilter === MIN_FILTER.LINEAR ||
            ...
            if (
                textureConfig.sampler.wrapS === TEXTURE_WRAP.CLAMP_TO_EDGE ||
            ...
            if (
                textureConfig.sampler.wrapT === TEXTURE_WRAP.CLAMP_TO_EDGE ||
