...
const KhronosPBRFragSrc:string = `#version 300 es
#ifdef GL_ES
...
vec3 getNormal()
{
    ...
    t =  (t * (1.0 - u_HasTangents)) + (tbn[0].xyz * u_HasTangents); 
    vec3 b = normalize(cross(ng, t));
    tbn[1].xyz * u_HasTangents + normalize(cross(ng, t)) * (1.0 - u_HasTangents);
...
void main(){
    ...
    vec4 baseColor = SRGBtoLINEAR(texture(u_BaseColorSampler, v_Texcoord)) * u_BaseColorFactor;
    ...
    vec3 l = normalize(u_LightPosition); // Vector from surface point to light
    ...
    vec3 emissive = SRGBtoLINEAR(texture(u_EmissiveSampler, v_Texcoord)).rgb * u_EmissiveFactor;
    color += emissive;
...
const KhronosPBRVertexSrc:string = `#version 300 es
//Vertex Attributes
...
export class KhronosPbrShader extends Shader {
    ...
    constructor(gl:WebGL2RenderingContext,cache:Cache){
        super(gl,KhronosPBRVertexSrc,KhronosPBRFragSrc);
        this.cache = cache;
    }
    ...
    public apply(material?:Material<MetallicRoughness>) {
        this.applyCamera();
    ...
    private applyTextures(material?:Material<MetallicRoughness>):void {
        ...
        if(specularEnv){
            ...
            console.log("Bind u_SpecularEnvSampler to location",  TEXTURES.SPECULAR_ENVIRONMENT)
        }else{
        ...
        if(diffuseEnv){
            ...
            console.log("Bind DiffuseEnvSampler to location",  TEXTURES.DIFFUSE_ENVIRONMENT);
        }else{
